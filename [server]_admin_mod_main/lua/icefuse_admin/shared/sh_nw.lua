...
File Path:   addons/[server]_admin_mod_main/lua/icefuse_admin/shared/sh_nw.lua
...
if IcefuseAdmin.NW then return end -- Do not ever lua refresh this file unless you want to lose data
...
function IcefuseAdmin.NW.Register(var, info) -- You must always call this on both the client and server. It will serioulsy break shit if you don't.
	local t = {
		Name = var,
...
function IcefuseAdmin.NW.IsRegistered(var)
	return (vars[var] ~= nil)
end
...
function IcefuseAdmin.NW.GetGlobal(var)
	return globals[var]
end
...
if (SERVER) then
	util.AddNetworkString 'IcefuseAdmin.NW.PlayerSync'
	util.AddNetworkString 'IcefuseAdmin.NW.NilEntityVar'
	util.AddNetworkString 'IcefuseAdmin.NW.NilPlayerVar'
	util.AddNetworkString 'IcefuseAdmin.NW.EntityRemoved'
	util.AddNetworkString 'IcefuseAdmin.NW.PlayerRemoved'

	net.Receive('IcefuseAdmin.NW.PlayerSync', function(len, pl)
	...
	hook.Add('EntityRemoved', 'IcefuseAdmin.NW.EntityRemoved', function(ent)
		local index = ent:EntIndex()
		if (index ~= 0) and (data[index] ~= nil) then -- For some reason this kept getting called on Entity(0), not sure why...
			if ent:IsPlayer() then
				net_Start('IcefuseAdmin.NW.PlayerRemoved')
					net_WriteUInt(index, 7)
				net_Broadcast()
			else
				net_Start('IcefuseAdmin.NW.EntityRemoved')
	...
	function IcefuseAdmin.NW.WaitForPlayer(pl, cback)
		if (pl.EntityCreated == true) then
			cback(pl)
		else
			if (callbacks[pl] == nil) then
	...
	function IcefuseAdmin.NW.SetGlobal(var, value)
		globals[var] = value
		if (value ~= nil) then
			vars[var]:_Send(0, value)
		else
			net_Start('IcefuseAdmin.NW.NilEntityVar')
				net_WriteUInt(0, 12)
	...
	function ENTITY:SetNetVar(var, value)
		...
		if (value ~= nil) then
			...
			if self:IsPlayer() then
				net_Start('IcefuseAdmin.NW.NilPlayerVar')
				net_WriteUInt(index, 7)
			else
				net_Start('IcefuseAdmin.NW.NilEntityVar')
				net_WriteUInt(index, 12)
	...
	hook.Add('InitPostEntity', 'IcefuseAdmin.NW.InitPostEntity', function()
		net_Start('IcefuseAdmin.NW.PlayerSync')
		net_Send()
	end)

	net.Receive('IcefuseAdmin.NW.NilEntityVar', function()
	...
	net.Receive('IcefuseAdmin.NW.NilPlayerVar', function()
	...
	net.Receive('IcefuseAdmin.NW.EntityRemoved', function()
		data[net_ReadUInt(12)] = nil
	end)

	net.Receive('IcefuseAdmin.NW.PlayerRemoved', function()
